...
from alertas import enviar_resumen_telegram, enviar_top_racha_telegram, enviar_modelos_tendencia_telegram
    ...
    if not mensajes:
        ...
        for m in mensajes:
            ...
            if texto in ("/resumen", "/start", "resumen"):
                ...
                print(f"Comando recibido de {chat_id_origen}: {texto}. Generando modelos en tendencia...")
                if not os.path.exists(ARCHIVO_HISTORIAL):
                    from telegram_bot import enviar_mensaje
                    enviar_mensaje("Todavía no hay ningún escaneo guardado.", chat_id=chat_id_origen)
                else:
                    filas = cargar_historial()
                    enviar_modelos_tendencia_telegram(filas, chat_id=chat_id_origen, dias=7)
            else:
